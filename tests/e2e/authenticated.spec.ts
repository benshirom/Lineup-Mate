...
import { ensureFirstActIsStarred, ensureFirstFestivalIsSaved, login, navigateTo, openFirstFestival } from './helpers';
...
test.describe('authenticated flows', () => {
  ...
  test('user can save a festival and see it on My Schedule', async ({ page }) => {
    ...
    await navigateTo(page, /My Schedule|הלוח שלי/i);
  ...
  test('user can open groups area', async ({ page }) => {
    await navigateTo(page, /Groups|קבוצות/i);
  ...
  test('user can open Profile page from the user badge', async ({ page }) => {
    // user-profile-link is on desktop; on mobile navigate via menu
    const profileLink = page.getByTestId('user-profile-link');
    if (await profileLink.isVisible().catch(() => false)) {
      await profileLink.click();
    } else {
      await navigateTo(page, /Profile|פרופיל/i);
    }
